...
async function generate() {
  ...
  // info and servers are replaced below
  const { openapi, info, servers, paths, ...rest } = result.data;
  const taggedPaths = {};
  for (const [route, operations] of Object.entries(paths)) {
    taggedPaths[route] = {};
    for (const [method, operation] of Object.entries(operations)) {
      // tags set in a #swagger comment take precedence
      taggedPaths[route][method] = { ...operation, tags: operation.tags || [tagFor(route)] };
    }
  }
