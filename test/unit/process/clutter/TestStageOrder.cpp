...
int main()
{
  ...
  // The clutter filter must not touch the reference channel.
  {
    ...
    WienerHopf filter(delayMin, delayMax, n, blah2::kFrontStageThreads);
  ...
  // Spectrum output is the same whichever side of the filter it runs on.
  {
    ...
    WienerHopf filterA(delayMin, delayMax, n, blah2::kFrontStageThreads);
    filterA.process(&xa, &ya);

    IqData xb(n), yb(n);
    fill(&xb, &yb, n);
    WienerHopf filterB(delayMin, delayMax, n, blah2::kFrontStageThreads);
  ...
  // be no worse.
  {
    ...
    const std::vector<Complex> serial = transform(blah2::kPlannerThreads);
    ...
    std::printf("   planner %d threads vs %d: %.3g of peak (%.0f dB below)\n",
                blah2::kPlannerThreads, blah2::kBackStageThreads, relative,
