...
- The API in `api/server.js` is described by `api/openapi.json`, an OpenAPI document generated from its routes by [swagger-autogen](https://swagger-autogen.github.io/docs/). After changing a route, regenerate it with `npm ci && npm run openapi` in `api/` and commit the result; CI (`.github/workflows/api.yml`) fails while the committed copy is stale. Summaries, descriptions and schemas can be added per route with `#swagger` comments in `server.js`.
